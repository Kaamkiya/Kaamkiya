```
    _____________________
   |                     |
   | Hello! I'm Kaamkiya |
   |_____________________|
        ||
(\___/) ||
(• ^ •) ||
/      =u
```

### about

Hello! I'm Kaamkiya, a backend developer who likes chocolate and terminals. I
mostly use Go, with some Zig and Python too.
Feel free to contribute to my projects, they're all open for help.

### skills
- Languages:   `go`, `zig`, `py`, `js`, `ts`, `html`, `css`
- Learning:    `zig`, `java`
- Databases:   `sqlite`, `postgres`
- Javascript:  `chartjs`, `p5js`, `astro`, `bun`, `deno`, `fresh`
- Libraries:   `processing`, `flask`, `go-chi`, `and many more`
- Distros:     `void`, `mint`, `debian`
- Other stuff: `linux`, `vim`, `neovim`

### social
- :elephant: Mastodon:    <a href="https://hachyderm.io/@nonexistent">@nonexistent</a>
- :mouse: Lemmy:          <a href="https://lemmy.ml/u/kaamkiya">@kaamkiya</a>
- :newspaper: HackerNews: <a href="https://news.ycombinator.com/user?id=kaamkiya">kaamkiya</a> (karma: <!--S:HN_KARMA-->9<!--E:HN_KARMA-->)

### misc
- pronouns: he/him
- wpm:      <!--S:MT_WPM-->69.8<!--E:MT_WPM--> with <!--S:MT_ACCURACY-->97.0<!--E:MT_ACCURACY-->% accuracy
  <!--duolingo: < !--S:DUO_XP--><!--E:DUO_XP--> <!--XP-->
- codewars: <!--S:CW_HONOR-->137<!--E:CW_HONOR--> honor

### stats
- stars earned:   <!--S:STARS_EARNED-->48<!--E:STARS_EARNED--> :star:
- contributed to: <!--S:CONTRIBUTED_TO-->24<!--E:CONTRIBUTED_TO--> :handshake:
- issues opened:  <!--S:ISSUES_OPENED-->76<!--E:ISSUES_OPENED--> :bug:
- prs opened:     <!--S:PRS_OPENED-->51<!--E:PRS_OPENED--> :inbox_tray:
- repo count:     <!--S:REPO_COUNT-->16<!--E:REPO_COUNT--> :package:
- gist count:     <!--S:GIST_COUNT-->0<!--E:GIST_COUNT--> :pencil:
- languages:<!--S:LANGUAGES-->
    - Go: 66.03%
    - Python: 14.56%
    - HTML: 5.77%
    - AsciiDoc: 3.2%
    - JavaScript: 2.6%
    - Processing: 2.17%
    - Elvish: 1.27%
    - Shell: 0.87%
    - Brainfuck: 0.82%
    - C: 0.77%
<!--E:LANGUAGES-->
</pre>
